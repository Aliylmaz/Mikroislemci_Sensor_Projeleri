...
while (1) {
		        ...
		        for (int i = 0; i < 25; ++i) {  // Her segment için kisa sürelerle çok sayida güncelleme yap
        ...
        if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_10) == GPIO_PIN_RESET) { // Check if button is pressed
            direction = 1; // Up counting
        } else {
            direction = 0; // Down counting
        }
